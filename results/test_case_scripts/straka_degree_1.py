...
degree 1 finite elements are used in this configuration. Embedded DG options are
used for the transport of theta and wind.
"""

from argparse import ArgumentParser, ArgumentDefaultsHelpFormatter
from firedrake import (
    PeriodicIntervalMesh, ExtrudedMesh, SpatialCoordinate, Constant, pi, cos,
    Function, sqrt, conditional, as_vector, FunctionSpace, BrokenElement
)
from gusto import (
    Domain, IO, OutputParameters, SemiImplicitQuasiNewton, SSPRK3, DGUpwind,
    Perturbation, DiffusionParameters, InteriorPenaltyDiffusion,
    BackwardEuler, CompressibleParameters, CompressibleEulerEquations,
    CompressibleSolver, compressible_hydrostatic_balance, EmbeddedDGOptions
)
...
def straka_bubble(
    ...
    # Transport schemes
    Vu_brok = FunctionSpace(
        mesh, BrokenElement(domain.spaces('HDiv').ufl_element())
    )
    Vtheta_brok = FunctionSpace(
        mesh, BrokenElement(domain.spaces('theta').ufl_element())
    )
    u_opts = EmbeddedDGOptions(embedding_space=Vu_brok)
    theta_opts = EmbeddedDGOptions(embedding_space=Vtheta_brok)
