...
galewsky_jet_defaults = {
    'ncells_per_edge': 64,     # num of cells per cubed-sphere/icosahedron edge
    'dt': 150.0,               # 2.5 minutes
    'tmax': 6.*24.*60.*60.,    # 6 days
    'dumpfreq': 3456,          # once per 6 days with default options
...
def galewsky_jet(
        ...
        icosahedron=galewsky_jet_defaults['icosahedron']
):
    ...
    degree = 1
    hdiv_family = 'BDM' if icosahedron else 'RTCF'
    ...
    if dirname == galewsky_jet_defaults['dirname']:
        dirname = f'{dirname}_icosahedralsphere' if icosahedron else f'{dirname}_cubedsphere'
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        '--icosahedron',
        help=(
            "Whether to use the icosahedral sphere mesh or the cubed sphere mesh"
        ),
        action="store_true",
        default=galewsky_jet_defaults['icosahedron']
    )
