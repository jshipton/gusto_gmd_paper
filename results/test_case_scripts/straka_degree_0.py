...
degree 0 finite elements are used in this configuration, with recovery used for
...
from gusto import (
    ...
    CompressibleSolver, compressible_hydrostatic_balance, RecoverySpaces,
    BoundaryMethod
)
...
def straka_bubble(
    ...
    # Factor of 2 in mu thanks to recovery
    diffusion_params = DiffusionParameters(kappa=kappa, mu=2.0*mu0/delta)
    ...
    # Diffusion schemes
    diffusion_schemes = [
        BackwardEuler(domain, "u", options=u_opts),
        BackwardEuler(domain, "theta", options=theta_opts)
    ]
    diffusion_methods = [
