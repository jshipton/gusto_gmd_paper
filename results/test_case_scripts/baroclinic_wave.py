...
from gusto import (
    ...
    xyz_vector_from_lonlatr, compressible_hydrostatic_balance, RecoverySpaces,
    MeridionalComponent, ZonalComponent, RadialComponent, CourantNumber,
    Temperature, Pressure, InternalEnergy, PotentialEnergy, CompressibleKineticEnergy
)

dry_baroclinic_sphere_defaults = {
    'ncell_per_edge': 32,      # 48 is max
    'nlayers': 15,             # 15 is fine
    'dt': 900.0,               # 15 minutes
    'tmax': 10.*24.*60.*60.,   # 10 days
...
def dry_baroclinic_sphere(
        ...
        vorder=dry_baroclinic_sphere_defaults['horder'],
        horder=dry_baroclinic_sphere_defaults['vorder']
):
    ...
    # ------------------------------------------------------------------------ #
     # vertical order of finite element de Rham complex
    u_eqn_type = 'vector_advection_form'  # Form of the momentum equation to use
    if horder == 0:
        ncell_per_edge = ncell_per_edge * 2
    if vorder == 0:
        nlayers = nlayers * 2
    order = (horder, vorder)
    ...
    diagnostic_fields = [
        ...
        Temperature(eqn), Pressure(eqn)
    ]
    io = IO(domain, output, diagnostic_fields=diagnostic_fields)

    recovery_spaces = ((0, 0), (0, 1), (1, 0))
    if order in recovery_spaces:
        Recovery_options=RecoverySpaces(domain)
        u_opts = Recovery_options.HDiv_options
        rho_opts = Recovery_options.DG_options
        theta_opts = Recovery_options.theta_options
    ...
    solver_options = {'ksp_converged_reason': None,
    ...
    compressible_hydrostatic_balance(
        eqn, theta0, rho0, exner_boundary=exner, solve_for_rho=True, params=solver_options)
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        '--horder',
        help='The order of the approximating polynomials in the horizontal direction',
        type=int,
        default=dry_baroclinic_sphere_defaults['horder']
    )
    parser.add_argument(
        '--vorder',
        help='The order of the approximating polynomials in the vertical direction',
        type=int,
        default=dry_baroclinic_sphere_defaults['vorder']
    )
