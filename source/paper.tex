...
Compatible finite elements %!TB methods?
provide a route to stable, consistent
discretisations of the partial differential equations governing
geophysical fluids %!TB cite acta numerica
%!TB Suggest rewriting and combining the following two sentences:
which also have the correct wave propagation speeds
and avoid spurious computational modes, even on non-uniform
grids.
These properties, along with conservation of mass and
...
and climate models \citep{staniforth2012horizontal}.
Achieving these
properties on a non-orthogonal grid is important because of parallel
scaling**. %!TB citation and expand on this. Reference Met Office and GungHo
While the required properties of the discretisation guide the
...
best for any particular problem.
%!TB Maybe a bit more direct about what Gusto is and why?
Gusto provides a flexible,
...
cells, or elements. %!TB: what does conforming mean here? Can we clarify what we mean by element? Do we want to say something about the elements being the same across the mesh?
Within each element, the field is represented as a
...
function space for each field. %!TB: should we distinguish between finite elements and function spaces?
Using different finite element function
...
Rham complex such that instead $\nabla$ maps onto the
...
enables schemes with local mass conservation \citep{}. %!TB: citation
...
implementation of the methods is strongly desirable. %!TB: should we change this sentence to start with the Met Office link? "Because of these promising properties, gungho. However there are still many important avenues for research and unanswered questions
%! - time stepping methods
%! - solver techniques
%! - aspects of physics-dynamics coupling
This is
...
different formulations of the equation. %!TB: this sentence felt a bit clunky. I guess we want to say that "The research of these questions can often be difficult in operational models, which are generally designed to execute specific configurations very efficiently. So the purpose of Gusto is to facilitate research by providing a flexible, extensible framework, in which the implementation of new methods is easier.
For geophysical fluid dynamics
...
for theta space) %!TB: this point feels a bit out of place here, should we move it above?
...
order to demonstrate the similarities between the equations and justify
