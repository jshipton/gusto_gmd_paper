...
\Author[4]{Timothy}{Andrews}
\Author[5]{Jack}{Betteridge}
\Author[1, 2]{Alex}{Brown}
\Author[6]{Thomas}{Gibson}
\Author[1]{Nell}{Hartney}
\Author[3]{Josh}{Hope-Collins}
\Author[1]{Dan}{Witt}

\affil[1]{Department of Mathematics and Statistics, University of Exeter}
\affil[2]{Met Office}
\affil[3]{Imperial College}
\affil[4]{University of Michigan}
\affil[5]{}
\affil[6]{}
...
\begin{abstract}
Gusto is a python code library for building dynamical cores that are
the heart of weather and climate prediction models. Gusto is built on
top of Firedrake which provides the software infrastructure required
to solve partial differential equations using the finite element
method. The compressible Euler equations solved by the dynamical core
have a particular mathematical structure that should be preserved by
the spatial discretisation. Recent research has shown that one way to
do this is using compatible finite element methods which also have the
advantage that this structure is preserved even on non-orthogonal
grids, enabling better parallel scaling through spatial domain
decomposition. This was the motivation for the development of the Met
Office new dynamical core, LFRic. However, there are different
families of compatible finite element method that can be defined on
different grids, such as the cubed sphere or icosahedral sphere
grids. For each family the different prognostic fields are discretised
using finite element spaces that have different orders and
interelement continuity. These function spaces can be difficult to
construct, in particular on the sphere where the Piola transform is
required to ensure that the horizontal wind vector is tangential to
the sphere. In addition, when simulating flows that have a much larger
horizontal scale than vertical scale, it is advantageous to have
meshes that are aligned in vertical columns; Firedrake provides such
meshes and Gusto provides the correct finite element families for the
compatible finite element method and a range of algorithms for stable
and accurate timestepping fields discretised on these finite element
spaces.

Gusto is intended as a tool for rapid prototyping of new algorithms,
based on compatible finite element methods, for dynamical cores. In
this context, the development of new algorithms typically follows a
path where they are tested on simpler equation sets such as the
shallow water equations or the Boussinesq equations. These equation
sets share important features of the mathematical structure of the
compressible Euler equation set and so provide a good indication of
whether new algorithms will work in a dynamical core, but they provide
a simpler setting for investigating the properties of the
algorithms. Therefore Gusto provides compatible finite element spatial
discretisations of these equation sets and then the timestepping
algorithms are written in such a way that they can be applied to any
of these equation sets. ** write a bit more about what this enables?
** talk about physics?

In this paper we describe the algorithms implemented in Gusto and
demonstrate their accuracy (and efficiency?) when applied to the
hierarchy of test problems commonly used in the numerical weather
...
\authorcontribution{JS and TMB conceptualisation, investigation, methodology, project administration, software, supervision, validation and writing. JS and CJC funding acquisition, CJC conceptualisation, investigation, methodology, project administration, software, supervision, TA, AB, NH, DW software and validation, JB, TG, JH-C software.} %% this section is mandatory
