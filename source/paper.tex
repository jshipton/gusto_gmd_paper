...
by the flexibility of Gusto? We summarise our current status and future
...
meaning that all the terms appear on the left hand side. We will
follow this convention below in contrast to many papers which present
the equations with the pressure gradient term and any forcing terms on
the right hand side.
...
where $\MM{u}$ is a three dimensional velocity field, $b$ is the fluid
buoyancy and $p$ is pressure. In terms of these variables, the
equations are:
...
acoustic wave speed. The velocity $\MM{u}$ can be two or three
dimensional, depending on whether the domain is a vertical slice
described by $(x, z)$ coordinates, of a three dimensional domain
...
typically taken to be *, $R$ is ** and $\kappa$ is **. The velocity
$\MM{u}$ can be two or three dimensional, depending on whether the
domain is a vertical slice described by $(x, z)$ coordinates, of a
three dimensional domain described by $(x, y, z)$ coordinates.
...
this means that the discretisation of these terms can be written in a
general way that means it can be applied to any of the equation sets.
...
facet. The brackets $\jump{q} = q^+-q^-$ denote the jump in a value
...
which manipulations to apply to which forms by applying filters. Here
we will describe the objects required to define labels and filters
before illustrating this with a simple example(s?).

\subsection{Form Manipulation Language Objects}

The finite element discretisation of each term in an equation is
represented in Firedrake by a \texttt{ufl.Form} object. FML defines a
\texttt{Term} object that comprises the \texttt{ufl.Form} defining the
finite element discretisation of the term and a dictionary of
key-value pairs corresponding to the FML \texttt{Label} objects that
apply to this term. An FML \texttt{Label} object has a name and an
optional value. When called on a \texttt{ufl.Form} object, the
\texttt{Label} ** Labels can also be used to tag parts of the
form. For example, the \texttt{subject} label is used to tag the
\texttt{Function} in the \texttt{ufl.Form} that represents the
prognostic variable. These operations are composable, so more than one
label can be attached to a term.

The end result is a \texttt{LabelledForm} object that holds a list of
terms, pairing individual \texttt{ufl.Form} objects with an FML
\texttt{label}. Labelled forms can be added together, multiplied by
floats and have additional terms added. More complex manipulations are
...
**describe methods has_label and get
...
applies an explicit step of forward Euler to the transport term and
an implicit step of backward Euler to the diffusion term:
\begin{align}
...
define and label the weak form of the residual; 2) build the solvers
for the explicit and implicit systems; 3) run the timestepping
loop. Since the intention of this example is to demonstrate FML rather
than the compatible finite element discretisation, we use space of piecewise
quadratic continuous Lagrange polynomials, denoted by $P_2$, for the finite
element discretisation as this greatly simplifies the forms. The
finite element discretisation of equation \ref{eq:1dadvdiff} is then
...
the residual, i.e. the left hand side of equation
\ref{eq:weak-advdiff}, using the FML labels \texttt{time\_derivative},
\texttt{transport} and \texttt{diffusion} to tag the corresponding
terms. The final line labels the function \texttt{q} as the subject of
this residual, i.e. the prognostic field. Note that the labels used
for tagging just take the UFL form as their argument and their value is
simply the default, \texttt{True}, in contrast to the \texttt{subject}
label that has the value \texttt{q}.

\lstinputlisting[caption=blah, label={lst:define-residual}, language=python, firstline=24, lastline=31]{code/advection_diffusion_splitting_example.py}
...
\ref{lst:define-explicit} we create the left and right hand sides of
equation \ref{eq:explicit-advdiff}. The left hand side should only
contain the time derivative term so the filter checks if each term has
the label \texttt{time\_derivative}, keeping the one that does and
dropping all others. To make this the left hand side of a
...
case simply a trial function \texttt{q\_trial}. \JScomment{Is the
...
\lstinputlisting[caption=blah, label={lst:define-explicit}, language=python, firstline=38, lastline=53]{code/advection_diffusion_splitting_example.py}

\lstinputlisting[caption=blah, label={lst:define-implicit}, language=python, firstline=56, lastline=70]{code/advection_diffusion_splitting_example.py}

\lstinputlisting[caption=blah, label={lst:timeloop}, language=python, firstline=56, lastline=70]{code/advection_diffusion_splitting_example.py}
