...
constructed by choosing the different finite element function spaces
...
compatible finite element framework to choose function spaces that
...
in figures \ref{fig:2D de Rham} and \ref{fig:3D de Rham}. In these
...
\begin{figure}\label{fig:2D de Rham}
  \centering{\includegraphics{de_rham_fig_2d.pdf}}
\end{figure}

\begin{figure}\label{fig:3D de Rham}
  \centering{\includegraphics{de_rham_fig_3d.pdf}}
\end{figure}

Given that there are many promising avenues for research in the area
of compatible finite element discretisations for geophysical fluid
dynamics (**where to say what these are?), a flexible, extensible
implementation of the methods is strongly desirable. This is
especially required because, while it is feasible for someone to write
a code implementing a specific finite element discretisation of a PDE,
the code quickly becomes complicated and unsustainable when
investigating different orders or families of finite elements, or
different formulations of the equation. For geophysical fluid dynamics
applications there are additional considerations relating to the
geometry and types of waves present in the flow. For example, the
scales of motion in the horizontal and vertical directions differ by
several orders of magnitude. (also: boundaries, vertical staggering
for theta space)
...
terms (e.g. pressure gradient, transport, diffusion) seen in section
\ref{sec: governing}. In section \ref{sec: design} we describe the
...
As these options can be combined in so many ways, it is only possible to present results from a tiny fraction configurations.
...
The term described chemical reaction is separated from the using a simple sequential splitting, \textcolor{red}{as described in Section \?}. \\
\\
...
\citet{williamson1992standard}) with the linear shallow water equations.
...
\\
We show results from the unstable jet of \citep{galewsky2004initial} in Figure \ref{fig:galewsky}, using the non-linear shallow water equations.
These results use the Semi-Implicit Quasi-Newton timestepper described in Section \ref{???}, and compare the RT elements on a cubed-sphere mesh with the BDM elements on an icosahedral-sphere.
...
\subsection{Compressible Euler equations}
We have also implemented many of the standard test cases for the non-hydrostatic compressible Euler equations that are used in modern NWP models.
...
Throughout this section we use the Semi-Implicit Quasi-Newton timestepper.
 \\
\\
The first test is the falling density current of \cite{straka1993numerical} in a vertical slice. This test involves the addition of diffusion to both the potential temperature and velocity equations.
...
The initial conditions and moisture parametrisation are those of \citep{bendall2020compatible}, which is an extension of the dry case of \citep{kelly2012continuous} and \citep{melvin2019mixed}.
Cross-sections of the initial and final equivalent potential temperature field are shown in Figure \ref{fig:3d_bubble}.
As in the two-dimensional test of \citep{bryan2002benchmark}, the atmosphere is saturated and cloudy everywhere, so that the moist processes are active in the whole domain.
\\
\begin{figure}[htp!]
\centering
\includegraphics[width=0.9\textwidth]{moist_bryan_fritsch.png}
\caption{Vertical cross-sections of the (left) initial and (right) final equivalent potential temperature field from the three-dimensional moist rising thermal test.
}
\label{fig:3d_bubble}
\end{figure}
\\
Our final test case is the deep baroclinic wave of \citep{ullrich2014proposed} on the sphere.
...
The surface pressure and temperature fields after 10 days are shown in Figure \ref{fig:baroclinic}, which compare well with those in \citep{melvin2024mixed} and demonstrate that our methods can replicate results from the literature.
