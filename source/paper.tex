...
define and label the residual; 2) build the explicit and implicit
solvers; 3) run the timestepping loop. Since the intention of this
example is to demonstrate FML rather than the compatible finite
element discretisation, we use piecewise quadratic continuous Lagrange
polynomials, $P_2$, for the finite element discretisation as this
greatly simplifies the forms. The finite element discretisation of
equation \ref{eq:1dadvdiff} is then
\begin{equation}\label{eq:weak-advdiff}
  \int_\Omega vq \diff x + c \int_\Omega v q_x \diff x + \kappa \int_\Omega v_x q_x \diff x = 0 \quad \forall v \in P_2,
\end{equation}
...
%\begin{align}\label{eq:weak-advdiff}
...
%\end{align}
%where $v$ is the test function and the diffusion term has been
%integrated by parts.
...
this residual. Note that the labels used for tagging just take the UFL
form as an argument and their value is simply the default, \texttt{True}, in
contrast to the \texttt{subject} label that has the value \texttt{q}.
...
To define the solvers for the explicit and implicit steps, we make use
of the predefined mappings \texttt{drop} and \texttt{keep} 
